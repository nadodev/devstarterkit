...
@section('title', 'Nova Tarefa - DevStarter Kit Demo')
@section('page-title', 'Nova Tarefa')
@section('page-description', 'Criar uma nova tarefa para o projeto')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-8 border border-gray-200/50">
        <form action="{{ route('demo.tasks.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <select id="project_id" 
                    ...
                    @foreach($projects as $project)
                        <option value="{{ $project['id'] }}" 
                                {{ (old('project_id', $selectedProject ?? '') == $project['id']) ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea id="notes" 
                          ...
                          placeholder="Observações adicionais sobre a tarefa">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
